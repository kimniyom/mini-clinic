...
class Thaibaht {
    ...
    public static function convert($amount = null) {
        if ((int) $amount == 0)
            return 'ศูนย์บาท';
    ...
    protected static function convertSegment($segment) {
        ...
        $last_digit = $length - 1;
        if ($length == 1)
            return static::$numbers[(int) $segment];
        $text = '';
        for ($nth = $last_digit; $nth >= 0; $nth--) {
            ...
            if ($nth === $last_digit) {
                $digit = '';
                $number = ($segment[$nth] == '1' and $segment[$nth - 1] != '0') ? static::$one_at_oneth : static::$numbers[(int) $segment[$nth]];
            }
            // tenth digit
