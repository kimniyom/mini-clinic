...
<div id="box-data">
    <table class="table table-bordered table-striped" id="p_product" style=" width: 100%;">
        <thead>
            <tr>
                <th style=" width: 5%;">#</th>
            ...
            foreach ($product as $last):
                ...
                if ($last['total'] > 0) {
                    ...
                    <tr>
                        ...
                        <td style=" padding: 0px;"><a href="javascript:movestock('<?php echo $last['id'] ?>','<?php echo $last['product_id']; ?>','<?php echo $last['lotnumber'] ?>','<?php echo $last['product_name']; ?>','<?php echo $last['total'] ?>')" class="btn btn-default btn-block" style=" margin: 0px; font-size: 18px; border-radius: 0px;"><i class="fa fa-exchange"></i> โยกสินค้า</a></td>
                    </tr>
                <?php } ?>
            <?php endforeach; ?>
        </tbody>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="gridSystemModalLabel">โยกสินค้า(<span id="cut_productname"></span>)</h4>
            </div>
            <div class="modal-body">
                <input type="hidden" class="form-control" id="cut_id" />
                ...
                <input id='cut_number' placeholder="ตัวเลขเท่านั้น ..." class="form-control" type='text' onKeyUp="if (this.value * 1 != this.value)
                            this.value = '';" >
                <label>ต้นทาง</label>
                <select id="branch_start" class="form-control" disabled="disabled">
                    <option value="<?php echo $branch['id'] ?>"><?php echo $branch['branchname'] ?></option>
                </select>
                <label>ปลายทาง</label>
                <select id="branch_end" class="form-control">
                    <?php foreach ($branchlist as $bl): ?>
                        <option value="<?php echo $bl['id'] ?>" <?php echo ($bl['id'] == $branch['id']) ? "disabled" : "" ?>><?php echo $bl['branchname'] ?></option>
                    <?php endforeach; ?>
                </select>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-success btn-block" onclick="sendproduct()">ยืนยัน</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function movestock(id, product_id, lotnumber, productname, total) {
        $("#cut_id").val(id);
    ...
    function sendproduct() {
        var url = "<?php echo Yii::app()->createUrl('clinicstoreproduct/moveproduct') ?>";
        var id = $("#cut_id").val();
        ...
        var branch_start = $("#branch_start").val();
        var branch_end = $("#branch_end").val();
        if (number == "") {
            $("#cut_number").focus();
            return false;
        }
